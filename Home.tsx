    ...
    <section className="min-h-screen bg-clean relative overflow-hidden flex flex-col">
      ...
      <div className="flex-grow flex flex-col items-center justify-center relative pt-24 pb-12 px-6">
        ...
        <div className="container mx-auto text-center z-10">
          <div className="inline-flex items-center gap-2 glass px-4 py-1.5 rounded-full mb-6 animate-in fade-in slide-in-from-top duration-700">
            {isAdmin ? <ShieldAlert size={12} className="text-blue-600" /> : <Sparkles size={12} className="text-yellow-500" />}
            <span className="text-[9px] sm:text-xs font-black uppercase tracking-widest text-slate-600">
              {isAdmin ? `Administrator Access` : `Welcome, ${firstName}!`}
            </span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center animate-in fade-in slide-in-from-bottom duration-1000 delay-300">
            ...
            <button onClick={() => onExplore('members')} className="px-10 py-5 glass text-slate-900 rounded-full hover:bg-white/80 transition-all text-xs font-bold uppercase tracking-widest">
              Squad List
            </button>
          </div>
        </div>
      ...
      {/* Mini Expertise (Only top 3 for cleaner landing) */}
