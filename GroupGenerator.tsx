        ...
        <section className="min-h-screen pt-32 pb-20 px-4 md:px-6 bg-clean relative overflow-hidden">
            ...
            <div className="container mx-auto max-w-6xl">
                <header className="text-center mb-12 animate-in slide-in-from-top duration-700">
                    <div className="inline-flex items-center gap-2 glass px-5 py-2 rounded-full mb-5 hover:scale-105 transition-transform">
                ...
                {isAdmin ? (
                        ...
                        <div className="glass rounded-[2.5rem] p-6 md:p-8 shadow-2xl border-white/60 relative overflow-hidden group hover:border-emerald-200 transition-colors">
                            ...
                            <div className="flex flex-col gap-6">
                                <div className="space-y-3">
                                    ...
                                    {/* Slider Diperbesar area sentuhnya untuk mobile */}
                                    <div className="relative h-6 w-full bg-slate-100 rounded-full overflow-hidden">
                                        <input 
                                            ...
                                            className="absolute w-full h-full opacity-0 cursor-pointer z-20 touch-manipulation"
                                        />
                                        <div 
                                ...
                                <button 
                                    ...
                                    disabled={isSpinning}
                                    className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] sm:text-xs flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl disabled:opacity-70 group active:scale-95 hover:shadow-2xl"
                                >
                    ...
                    <div className="text-center p-8 glass rounded-[2.5rem] mb-12 max-w-lg mx-auto border-dashed border-2 border-slate-200 hover:border-slate-300 transition-colors">
                ...
                {!isSpinning && groups.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 animate-in slide-in-from-bottom duration-700">
                        {groups.map((group, idx) => (
                            <div 
                                key={group.id} 
                                className="glass rounded-[2rem] p-6 shadow-lg border-white/60 hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 hover:-translate-y-2 relative overflow-hidden group-card"
                                style={{ animationDelay: `${idx * 100}ms` }}
                            >
                                {/* Decorative blob */}
                                <div className="absolute -right-6 -top-6 w-24 h-24 bg-gradient-to-br from-emerald-100 to-teal-50 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                                ...
                                <div className="relative z-10">
                                            ...
                                            {isAdmin ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-full bg-transparent font-artist text-xl font-black text-slate-900 outline-none border-b border-transparent focus:border-emerald-300 uppercase tracking-tight placeholder:text-slate-300 truncate"
                                                    />
                                                </div>
                                            ) : (
                                                <h3 className="font-artist text-xl font-black text-slate-900 uppercase tracking-tight truncate">{group.name}</h3>
                                            )}
                                        </div>
                                    ...
                                    <div className="space-y-2.5">
                                        {group.members.map((member, mIdx) => (
                                            <div key={mIdx} className="flex items-center gap-3 group/item">
                                                <div className="w-6 h-6 shrink-0 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-[8px] font-black text-slate-400 group-hover/item:bg-emerald-500 group-hover/item:text-white group-hover/item:border-emerald-500 transition-colors shadow-sm">
                                                    {mIdx + 1}
                                                </div>
                                                <span className="text-[11px] font-bold text-slate-600 uppercase tracking-tight truncate group-hover/item:text-slate-900 transition-colors w-full block">
                                                    {member}
                                                </span>
                                            </div>
                                        ))}
                                    ...
                                    <div className="mt-6 flex justify-end">
                                        <Sparkles size={12} className="text-slate-200 group-hover:text-emerald-400 transition-colors animate-pulse" />
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                {!isSpinning && groups.length === 0 && (
                    <div className="text-center py-20 opacity-40 hover:opacity-100 transition-opacity duration-500">
