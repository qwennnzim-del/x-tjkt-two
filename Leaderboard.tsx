...
import { Medal, Crown, Sparkles, Loader2, Trophy, Users, ShieldCheck } from 'lucide-react';
...
const Leaderboard: React.FC = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, "leaderboard"), orderBy("score", "desc"), orderBy("createdAt", "asc"), limit(10));
        ...
        <div className="glass rounded-[3.5rem] p-6 md:p-12 shadow-3xl border-white/60 relative overflow-hidden">
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-artist text-xl font-black text-slate-900 uppercase tracking-tight leading-none">Top 10 Teknisi</h3>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Real-time Data Updates</p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              leaderboard.map((entry, i) => (
                <div 
                  key={entry.id} 
                  className={`flex items-center justify-between p-5 md:p-6 rounded-[2.5rem] transition-all duration-500 hover:scale-[1.02] border animate-in slide-in-from-bottom duration-500 ${
                ...
                >
                  <div className="flex items-center gap-5 md:gap-8">
                    <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center text-lg font-black shadow-inner transition-transform group-hover:rotate-6 ${
                      i === 0 ? 'bg-amber-400 text-white' : 
                      i === 1 ? 'bg-slate-400 text-white' :
                      i === 2 ? 'bg-orange-400 text-white' : 'bg-slate-100 text-slate-400'
                    }`}>
                      {i === 0 ? <Crown size={24} className="animate-pulse" /> : 
                       i === 1 ? <Medal size={24} /> :
                       i === 2 ? <Medal size={24} /> : i + 1}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {i < 3 && <Sparkles size={12} className="text-amber-400" />}
                      </div>
              ...
              <div className="py-24 text-center glass rounded-[2.5rem] border-dashed border-2 border-slate-200">
                ...
                <p className="text-slate-300 text-[10px] mt-2 uppercase tracking-[0.3em] font-black">Jadilah yang pertama mengisi papan skor!</p>
              </div>
            )}
          </div>
